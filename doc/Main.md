...
type **INTEGER NOT NULL**, **FOREIGN KEY** (type) **REFERENCES**
question_types(id)  
);  
...
id **INTEGER NOT NULL AUTO_INCREMENT**, **PRIMARY KEY** (id),  
question **INTEGER NOT NULL**, **FOREIGN KEY** (question) **REFERENCES**
questions(id),  
...
id **INTEGER NOT NULL AUTO_INCREMENT**, **PRIMARY KEY** (id),  
**session INTEGER NOT NULL**, **FOREIGN KEY** (**session**) **REFERENCES**
sessions(id)  
);  
...
id **INTEGER NOT NULL AUTO_INCREMENT**, **PRIMARY KEY** (id),  
note **VARCHAR**(256),  
answer **INTEGER NOT NULL**, **FOREIGN KEY** (answer) **REFERENCES**
answers(id),  
variant **INTEGER NOT NULL**, **FOREIGN KEY** (variant) **REFERENCES**
variants(id)  
);  
...
Poll poll = new Poll("Яка ти принцеса Дісней?", "bla-bla-bla, this is a test
description");  
List\<Question\> qs = poll.getQuestions();  
  
Question q = new Question(poll, 1, "Обери риси, що для тебе найбільше
притаманні:", *nOfMany*);  
qs.add(q);  
...
q = new Question(poll, 2, "Зазначте настільки ви згодні з наступними
висловлюваннями?", *matrix*);  
qs.add(q);  
md = q.getMetadata();  
vs = q.getVariants();  
...
private static void addMatrix(Question q, List\<Variant\> vs, List\<Variant\>
rows, List\<Variant\> columns) {  
vs.addAll(rows);  
if(columns != rows) {  
vs.addAll(columns);  
}  
